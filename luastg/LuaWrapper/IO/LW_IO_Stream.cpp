...
#define GETUDATA(index) StreamWrapper::Wrapper* p = static_cast<StreamWrapper::Wrapper*>(luaL_checkudata(L, (index), LUASTG_LUA_TYPENAME_IO_STREAM));
					...
					static int CanResize(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						lua_pushboolean(L, p->handle->CanResize());
						return 1;
					}
					static int GetLength(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						lua_Integer len = (lua_Integer)p->handle->GetLength();
						::lua_pushinteger(L, len);
						return 1;
					}
					static int SetLength(lua_State* L)LNOEXCEPT {
						GETUDATA(-2);
					...
					static int GetPosition(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						lua_Integer pos = (lua_Integer)p->handle->GetPosition();
						::lua_pushinteger(L, pos);
						return 1;
					}
					static int SetPosition(lua_State* L)LNOEXCEPT {
						GETUDATA(-3);
					...
					static int ReadBytes(lua_State* L)LNOEXCEPT {
						GETUDATA(-2);
					...
					static int WriteBytes(lua_State* L)LNOEXCEPT {
						GETUDATA(-2);
					...
					static int Lock(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						p->handle->Lock();
						return 0;
					}
					static int TryLock(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						lua_pushboolean(L, FCYERR_OK == p->handle->TryLock());
						return 1;
					}
					static int Unlock(lua_State* L)LNOEXCEPT {
						GETUDATA(-1);
						p->handle->Unlock();
						return 0;
					}
